...
// INDEX
// GET /profiles
router.get('/profiles', requireToken, (req, res) => {
  Profile.find()
    .then(profiles => {
      const profilesOwner = profiles.filter(profile => {
        if (JSON.stringify(profile.owner) === JSON.stringify(req.user._id)) {
          return true
        }
      })
      ...
      return profilesOwner.map(profile => profile.toObject())
    })
...
router.get('/profiles/:id', requireToken, (req, res) => {
...
// CREATE
// POST /profiles
router.post('/profiles', requireToken, (req, res) => {
...
router.patch('/profiles/:id', requireToken, (req, res) => {
...
router.delete('/profiles/:id', requireToken, (req, res) => {
  Profile.findById(req.params.id)
    .then(handle404)
    .then(profile => {
